...
impl Config {
    ...
    ///
    /// Currently runs the legacy `engine = "apple-speech"` →
    /// `[live_transcript] backend` migration and preserves a legacy
    /// auto-summarization default when the engine key is missing.
    ///
